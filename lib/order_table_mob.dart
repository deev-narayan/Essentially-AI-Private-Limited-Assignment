...
class OrdersTableMob extends StatefulWidget {
  const OrdersTableMob({super.key});

  @override
  State<OrdersTableMob> createState() => _OrdersTableMobState();
}

class _OrdersTableMobState extends State<OrdersTableMob> {
  // Sample JSON data
  ...
  Widget build(BuildContext context) {
    ...
    final bool isMobile = screenWidth < 665;
        ...
        child: DataTable(
          ...
          border: TableBorder(
            horizontalInside: BorderSide(
              color: const Color.fromARGB(38, 255, 255, 255).withOpacity(0.3),
              width: 0.5,
            ),
          ),
          columns: const [
          ...
          rows: orders.map((order) {
            return DataRow(
              cells: [
                ...
                DataCell(
                  Text(order["client"], style: const TextStyle(fontSize: 12)),
                ),
                DataCell(
                  Row(
                    children: [
                      ...
                      const Icon(Icons.sensors, size: 12, color: Colors.blue),
                    ],
                  ),
                ),
                DataCell(
                  Text(order["side"], style: const TextStyle(fontSize: 12)),
                ),
                DataCell(
                  Text(order["product"], style: const TextStyle(fontSize: 12)),
                ),
                DataCell(
                  Text(order["qty"], style: const TextStyle(fontSize: 12)),
                ),
                DataCell(
                  Text(order["price"], style: const TextStyle(fontSize: 12)),
                ),
                DataCell(
                  IconButton(
                    icon: const Icon(Icons.more_horiz, size: 12),
                    onPressed: () {},
                  ),
                ),
              ],
